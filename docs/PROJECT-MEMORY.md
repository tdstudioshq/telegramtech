...
- **Interpretations flagged for Tyler (Session 5):** (1) ContentUnlocked is raised by PurchaseService in the grant-minting tx (§6 diagram) not by DeliveryEngine (§9 table row conflicts; delivery only audits content.delivered — revisit if "first delivery" semantics needed). (2) Audit vocabulary extended with `purchase.failed` and enrichment namespace `event.purchase_completed`/`event.content_unlocked` (varchar-by-design accommodates; Zod list updated). (3) publishDrop requires ≥1 asset (invented guard: empty drop delivers nothing). (4) SubscriptionActivated is raised on renewal too (no separate renewal event exists; audit distinguishes via subscription.renewed).
- **Next session: M4 (Telegram adapter).** Do not start without Tyler. PurchaseService.failStalePending deliberately deferred to M5 (cleanup job milestone).
